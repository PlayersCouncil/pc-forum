...
$lang = array_merge($lang, array(
	...
	'CONTAINER_EXCEPTION' => '由於安裝擴充功能，phpBB 遇到建構容器的錯誤。因此，所有擴充功能已暫時停用。請嘗試清除您的討論區緩存。當容器錯誤解決後，所有擴充功能將自動重新啟用。如果此錯誤持續，請訪問 <a href="https://www.phpbb.com/support">phpBB.com</a> 尋求支援。',
	'EXCEPTION' => '例外',
	...
	'CSV_INVALID'			=> '提供的逗號分隔設定「%1$s」無效。應僅用逗號分隔，不應包含任何前導或尾隨的符號。',
	'CURRENT_VERSION'		=> '現行的版本',
	...
	'NO_EMAILS_DEFINED'		=> '沒有找到有效的電子郵件位址。',
	...
	'REMIND'							=> '提醒',
	'REPARSE_LOCK_ERROR'				=> '重新解析已經由另一個程序進行。',
	'RESYNC'							=> '重新同步',
	...
	'UCP'					=> '會員控制台 (UCP)',
	'URL_INVALID'			=> '設定所提供的網址「%1$s」是無效的。',
	'URL_SCHEME_INVALID'	=> '以逗號分隔的設定「%1$s」中提供的方法「%2$s」無效。應以拉丁字符開頭，後跟字母數字、連字符號或點。',
	'USERNAMES_EXPLAIN'		=> '請每一行輸入一個會員名稱。',
	'USER_CONTROL_PANEL'	=> '會員控制台',

	'UPDATE_NEEDED'			=> '討論區不是最新的。',
	'UPDATE_NOT_NEEDED'		=> '討論區是最新的。',
...
// PHP info
$lang = array_merge($lang, array(
	'ACP_PHP_INFO_EXPLAIN'	=> '這個頁面列出了這個伺服器上所安裝的 PHP 版本資訊。它包括了已裝載模組的細節，可用的變數和預設的設定。在診斷問題時這些資訊可能會有用。請注意！某些主機商可能會因為安全理由限制顯示某些資訊。建議您不要洩露這頁的任何細節，除非在支援討論區中被 <a href="https://www.phpbb.com/about/team/">官方團隊成員</a> 要求提供。',
...
$lang = array_merge($lang, array(
	...
	'ACP_CRITICAL_LOGS_EXPLAIN'	=> '這裡列出了討論區自身執行的所有動作。這些記錄提供您解決特定問題的有用資訊，例如；無法投遞的電子郵件。您可以按會員名稱、日期、IP 位址或動作排序。如果您有合適的權限，那麼您也可以清除部分或全部的記錄。',
...
$lang = array_merge($lang, array(
	...
	'ERROR_MBSTRING_HTTP_INPUT_EXPLAIN'				=> '<var>mbstring.http_input</var> 必須留空。您可以在 <samp>PHP 資訊</samp> 頁檢查目前的設定值。',
	'ERROR_MBSTRING_HTTP_OUTPUT'					=> 'HTTP 輸出的字元轉換之設定不當',
	'ERROR_MBSTRING_HTTP_OUTPUT_EXPLAIN'			=> '<var>mbstring.http_output</var> 必須留空。您可以在 <samp>PHP 資訊</samp> 頁檢查目前的設定值。',
	'ERROR_DEFAULT_CHARSET'							=> '預設字符集設定不正確',
	'ERROR_DEFAULT_CHARSET_EXPLAIN'					=> '<var>default_charset</var> 必須設定為 <samp>UTF-8</samp>。您可以在 <samp>PHP 資訊</samp> 頁檢查目前的設定值。',
	...
	'PHP_VERSION'		=> 'PHP 版本',
	'PHP_VERSION_OLD'	=> '您伺服器中的 PHP 版本（%1$s）將不再被未來的 phpBB 版本支援。最低要求的版本將是 PHP %2$s。%3$s詳細情形%4$s',
	...
	'VERSION_CHECK'					=> '版本檢查',
	'VERSION_CHECK_EXPLAIN'			=> '檢查您安裝的 phpBB 是否是最新的。',
	'VERSIONCHECK_INVALID_ENTRY'	=> '最新版本的資訊包含不受支持的條目。',
	'VERSIONCHECK_INVALID_URL'		=> '最新版本的資訊包含無效的網址。',
	'VERSIONCHECK_INVALID_VERSION'	=> '最新版本的資訊包含無效的版本。',
	'VERSION_NOT_UP_TO_DATE_ACP'	=> '您安裝的 phpBB 不是最新的。<br />以下是釋出公告的連結，其中包含更多資訊以及更新說明。',
	'VERSION_NOT_UP_TO_DATE_TITLE'	=> '您安裝的 phpBB 不是最新的。',
	'VERSION_UP_TO_DATE_ACP'		=> '您安裝的 phpBB 是最新的。目前沒有可用的更新。',
...
$lang = array_merge($lang, array(
	...
	'INACTIVE_USERS_EXPLAIN'		=> '這是已經註冊但是尚未啟用帳號的會員列表。如果您希望的話，那麼您可以啟用、刪除或提醒（發送電子郵件）這些會員。',
	'INACTIVE_USERS_EXPLAIN_INDEX'	=> '這是最近十個已經註冊但是尚未啟用帳號的會員列表。帳號是無效的，可能是因為會員註冊後需要啟用，而這些帳號尚未啟用，或者是因為這些帳號已被停用。您可以從適當的選單項目或下面的「檢視未啟用的會員」連結檢視完整列表，從那裡您可以啟用、刪除或提醒（發送電子郵件）這些會員。',
...
$lang = array_merge($lang, array(
	'EXPLAIN_SEND_STATISTICS'	=> '請發送您的伺服器以及討論區設定的資訊到 phpBB 以作為統計分析。所有您的鑑別以及您的網站資訊將被移除 - 整個資料都是 <strong>匿名的</strong>。這資訊將提供我們關於未來 phpBB 版本決策的依據。這統計資料是可以公開的。我們也與 PHP 計畫分享這資料，以做出 phpBB 編程語言。',
	...
	'SEND_STATISTICS_LONG'		=> '發送統計資訊',
	'SHOW_STATISTICS'			=> '顯示細節',
	'THANKS_SEND_STATISTICS'	=> '感謝您送出您的資訊。',
	'FAIL_SEND_STATISTICS'		=> 'phpBB 無法發送統計',
));
...
$lang = array_merge($lang, array(
	...
	'LOG_ACL_ADD_USER_LOCAL_F_'			=> '<strong>已增加或已編輯此會員的版面使用權</strong> 到 %1$s<br />» %2$s',
	'LOG_ACL_ADD_USER_LOCAL_M_'			=> '<strong>已增加或已編輯此會員的版面版主使用權</strong> 到 %1$s<br />» %2$s',
	'LOG_ACL_ADD_GROUP_LOCAL_F_'		=> '<strong>已增加或已編輯此群組的版面使用權</strong> 到 %1$s<br />» %2$s',
	'LOG_ACL_ADD_GROUP_LOCAL_M_'		=> '<strong>已增加或已編輯此群組的版面版主使用權</strong> 到 %1$s<br />» %2$s',

	'LOG_ACL_ADD_MOD_LOCAL_M_'			=> '<strong>已增加或已編輯版主</strong> 為 %1$s<br />» %2$s',
	'LOG_ACL_ADD_FORUM_LOCAL_F_'		=> '<strong>已增加或已編輯版面權限</strong> 為 %1$s<br />» %2$s',
	...
	'LOG_BBCODE_CONFIGURATION_ERROR'	=> '<strong>設定 BBCode 時出錯</strong>: %1$s<br />» %2$s',
	...
	'LOG_CONFIG_EMAIL'			=> '<strong>已更改電子郵件設定</strong>',
	...
	'LOG_ERROR_EMAIL'		=> '<strong>電子郵件錯誤</strong><br />» %s',
	'LOG_ERROR_CAPTCHA'		=> '<strong>確認代碼錯誤</strong><br />» %s',
	...
	'LOG_INACTIVE_REMIND'	=> '<strong>已發送提醒電子郵件給未啟用的會員</strong><br />» %s',
	...
	'LOG_MASS_EMAIL'		=> '<strong>發送大量電子郵件</strong><br />» %s',
	...
	'LOG_SPAMHAUS_OPEN_RESOLVER'		=> 'Spamhaus 不允許使用開放解析器進行查詢。黑名單檢查已被禁用。有關詳細信息，請參閱 https://www.spamhaus.com/product/help-for-spamhaus-public-mirror-users/。',
	'LOG_SPAMHAUS_VOLUME_LIMIT'			=> '已超出 Spamhaus 查詢量限制。黑名單檢查已被禁用。有關詳細信息，請參閱 https://www.spamhaus.com/product/help-for-spamhaus-public-mirror-users/。',
	...
	'LOG_USER_BAN_EMAIL'	=> '<strong>已封鎖電子郵件經由會員管理</strong>理由是「<em>%1$s</em>」<br />» %2$s',
	...
	'LOG_USER_UPDATE_EMAIL'	=> '<strong>已改變會員「%1$s」的電子郵件</strong><br />» 從「%2$s」到「%3$s」',
	...
	'LOG_WARNINGS_DELETED'		=> array(
		1 => '<strong>已刪除 %2$d 個會員警告</strong><br />» %1$s', // 例如：'<strong>已刪除 2 個會員警告</strong><br />» 會員名稱'
	),
	...
	'LOG_EXT_UPDATE'	=> '<strong>擴充功能已更新</strong><br />» %s',
));
