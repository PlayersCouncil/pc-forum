...
$lang = array_merge($lang, array(
	...
	'BBCODE_DANGER'				=> '您嘗試新增的 BBCode 似乎不安全。如果 BBCode 使用 {TEXT} 標記在敏感的上下文中，試試使用更多限制的型式取代。只有在您了解所涉及的風險時繼續操作。',
	'BBCODE_DANGER_PROCEED'		=> '繼續執行', //'我了解風險',
	...
	'BBCODE_INVALID_TEMPLATE'	=> '您的 BBCode 樣板無效。',
	'BBCODE_TAG'				=> '標籤',
	...
	'tokens'	=>	array(
		...
		'NUMBER'		=> '任何數字序列',
		'EMAIL'			=> '有效的電子郵件位址',
		'URL'			=> '有效的 URL 可使用任何允許的協議(如 http、ftp、...等，但不能使用 javascript 功能)。如果沒有提供，將將自動以「http://」當作字符串的開頭。',
		'LOCAL_URL'		=> '本地的 URL。URL 必須和主題頁相關，並且不能包含伺服器名稱或通訊協議，以「%s」做為連結的前綴。',
		'RELATIVE_URL'	=> '相對的 URL。您可以用它來匹配為 URL 的一部分，但要小心：一個完整的 URL 是一個有效而相對的 URL。當您的討論區想使用相對的 URL，得使用本地的 URL 之標誌符號。',
		...
		'ALNUM'			=> '拉丁字母（A-Z）中的字元和數字。',
		'CHOICE'		=> '指定值的選擇，例如<samp>{CHOICE=spades,hearts,diamonds,clubs}</samp>。預設情況下，這些值不會區分大小寫，也可以透過指定 <samp>caseSensitive</samp> 選項來區分大小寫：<samp>{CHOICE=Spades,Hearts,Diamonds,Clubs;caseSensitive}</samp>。',
		'FLOAT'			=> '十進位值，例如<samp>0.5</samp>。',
		'HASHMAP'		=> '將字串對應到其替換，格式為 <samp>{HASHMAP=string1:replacement1,string2:replacement2}</samp>。區分大小寫。預設保留未知值。',
		'INT'			=> '整數值，例如<samp>2</samp>。',
		'IP'			=> '有效的 IPv4 或 IPv6 位址。',
		'IPPORT'		=> '帶有連接埠號碼的有效 IPv4 或 IPv6 位址。',
		'IPV4'			=> '有效的 IPv4 位址。',
		'IPV6'			=> '有效的 IPv6 位址。',
		'MAP'			=> '將字串對應到其替換，格式為 <samp>{MAP=string1:replacement1,string2:replacement2}</samp>。不區分大小寫。預設保留未知值。',
		'RANGE'			=> '接受給定範圍內的整數，例如<samp>{RANGE=-10,42}</samp>。',
		'REGEXP'		=> '根據給定的正規表示式驗證其值，例如<samp>{REGEXP=/^foo\w+bar$/}</samp>。',
		'TIMESTAMP'		=> '時間戳，例如<samp>1h30m10s</samp>，它將轉換為秒數。也接受一個號碼。',
		'UINT'			=> '無符號整數值。與 <samp>{INT}</samp> 相同，但拒絕小於 0 的值。',
	),
));
